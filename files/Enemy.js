...
export class Enemy extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y, type='goon') {
    // Each type has its own texture and size
    const typeConfig = {
  ...
  takeHit(damage, attacker) {
    ...
    this.setVelocityX(dir * 220); this.setVelocityY(-180);

    if (this.health <= 0 && attacker?.score !== undefined) {
      attacker.score += this.scoreValue;
      this.scene.events.emit('score:updated', { playerIndex: attacker.playerIndex, score: attacker.score });
    }
  ...
  _die() {
    ...
    this._hpBar && this.scene.tweens.add({ targets: this._hpBar, alpha: 0, duration: 300 });
  }
  ...
  _drawHP() {
    ...
    const x = this.x - bw/2, y = this.y - 34;
  ...
  _buildAnims(key) {
    const frameCount = key === 'enemy' ? 4 : 4;

    if (!this.scene.anims.exists(`${key}_walk`)) {
      this.scene.anims.create({
        key: `${key}_walk`,
        frames: Array.from({ length: frameCount }, (_, i) => ({ key, frame: i })),
  ...
  destroy() { this._hpBar?.destroy(); this._nameTag?.destroy(); super.destroy(); }
}
